...
; Hypothesis: replace should modify the target series and replace only the first occurrence by default
; Hypothesis: replace should return the modified target series by default
...
; Hypothesis: /all should replace all occurrences of the search value
; Hypothesis: /all should still modify the target series and return it
...
; Hypothesis: /case should perform case-sensitive matching
; Hypothesis: without /case, matching should be case-insensitive by default
...
; Hypothesis: /tail should return the target positioned after the last replacement
; Hypothesis: /tail should still modify the original target series
...
; Hypothesis: replace should work with blocks as target series
; Hypothesis: search and replace values should work with any-type! in blocks
...
assert-equal [x b c a b c] result-block-1 "Block replacement return value"
...
assert-equal [99 2 3 99 2 3] result-block-2 "Block /all return value"
...
assert-equal [hi world hi] result-block-3 "Block word return value"

;;-----------------------------------------------------------------------------
;; Probing Multi-Element Block Search/Replace
;;-----------------------------------------------------------------------------
print "^/--- Probing Multi-Element Block Operations ---"
; Hypothesis: replace should handle multi-element sequences in blocks
...
assert-equal [x y c d a b e f] result-block-4 "Multi-element block return value"
...
assert-equal [x y c x y d x y e] result-block-5 "Multi-element /all return value"

;;-----------------------------------------------------------------------------
;; Probing Type Conversion Behavior
;;-----------------------------------------------------------------------------
print "^/--- Probing Type Conversion ---"
; Hypothesis: search values should be converted to match target series type when necessary
...
assert-equal "num 456 123" result-15 "Integer search conversion return value"
...
assert-equal "replacement word2 word1" result-16 "Word search conversion return value"

;;-----------------------------------------------------------------------------
;; Probing Edge Cases
;;-----------------------------------------------------------------------------
print "^/--- Probing Edge Cases ---"
; Hypothesis: Empty strings and empty blocks should be handled gracefully
; Hypothesis: Self-referential replacements should work correctly

test-string-17: ""
result-17: replace test-string-17 "anything" "something"
assert-equal "" test-string-17 "Empty string target unchanged"
assert-equal "" result-17 "Empty string return value"

test-string-18: "test"
result-18: replace test-string-18 "" "x"
assert-equal "test" test-string-18 "Empty search string no replacement"
assert-equal "test" result-18 "Empty search return value"

test-string-19: "hello"
result-19: replace test-string-19 "hello" ""
assert-equal "" test-string-19 "Replacement with empty string"
assert-equal "" result-19 "Empty replacement return value"

test-block-6: []
result-block-6: replace test-block-6 'anything 'something
assert-equal [] test-block-6 "Empty block target unchanged"
assert-equal [] result-block-6 "Empty block return value"

;;-----------------------------------------------------------------------------
;; Probing Large Replacement Operations
;;-----------------------------------------------------------------------------
print "^/--- Probing Large Replacement Operations ---"
; Hypothesis: replace should handle larger strings efficiently
; Hypothesis: replacing with longer strings should work correctly
...
assert-equal "much larger replacement much larger replacement much larger replacement" result-20 "Large replacement return value"
...
assert-equal "short" result-21 "Entire string replacement return value"

;;-----------------------------------------------------------------------------
;; Probing Function as Replacement Value
;;-----------------------------------------------------------------------------
print "^/--- Probing Function as Replacement Value ---"
; Hypothesis: replace should accept and call functions as replacement values
; Hypothesis: function should be called for each replacement
...
assert-equal "replacement1 replacement2 replacement3" result-22 "Function replacement return value"
