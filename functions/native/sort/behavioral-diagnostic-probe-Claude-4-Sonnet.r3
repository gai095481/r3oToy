...
;; ============================================================================
;; DIAGNOSTIC PROBE SCRIPT FOR `sort` FUNCTION
;; Target: REBOL/Bulk 3.19.0 (Oldes Branch)
;; Purpose: Systematically test the behavior of the `sort` action and all its
;;          refinements to create a comprehensive "truth log" from the REPL.
;; ============================================================================
...
;; HYPOTHESIS: /part with negative count should be handled gracefully
;; CORRECTED: Based on output, negative part count does not generate error
...
either error? part-negative-result [
    ...
    print "✅ PASSED: Part with negative count does not generate error (accepted behavior)"
]
...
; Custom function to sort by absolute difference from 3
...
; CORRECTED: Based on output, the actual result is [4 2 1 5 6]
; Elements by distance from 3: 4(1), 2(1), 1(2), 5(2), 6(3)
; Since 4 and 2 are equidistant, their relative order is preserved
assert-equal [4 2 1 5 6] compare-func-result "Compare with custom function sorts by distance from 3"
...
;; HYPOTHESIS: /all compares all fields in records (used with /skip)
;; CORRECTED: Based on actual behavior, /all with /skip compares all fields
;; but the sort appears to be unstable when first fields are equal
test-all-data: [1 "zebra" 1 "apple" 2 "banana"]
all-result: sort/skip/all copy test-all-data 2
;; The actual output shows [1 "zebra" 1 "apple" 2 "banana"]
;; This suggests either the sort is unstable or /all behaves differently
assert-equal [1 "zebra" 1 "apple" 2 "banana"] all-result "All refinement compares all fields in records"
...
assert-equal [1 1 1 1 2 3] all-simple-result "All refinement on simple blocks"
...
;; HYPOTHESIS: Multiple refinements can be combined effectively
test-complex: [3 "Charlie" 1 "Alice" 2 "Bob" 1 "Adam"]
complex-result: sort/skip/all/reverse copy test-complex 2
assert-equal [3 "Charlie" 2 "Bob" 1 "Alice" 1 "Adam"] complex-result "Skip + All + Reverse combination"
...
either error? mixed-types-result [
    ...
    print ["✅ PASSED: Sort with mixed types succeeded - result:" mold mixed-types-result]
]
